...
- **Live data is never remembered, always asked.** Today's calories, what's left of a batch, pending reminders, the current weight: run the command (a read stays with you unless you pass `--send`). A number you recall from earlier in the conversation is a number from the past.
