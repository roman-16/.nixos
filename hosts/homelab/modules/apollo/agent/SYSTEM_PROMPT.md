...
- **Live data is never remembered, always asked.** Today's calories, what's left of a batch, pending reminders, the current weight: run the command (with `--quiet` where the skill offers it and the answer is for you). A number you recall from earlier in the conversation is a number from the past.
