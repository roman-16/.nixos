...
describe("renderSummary", () => {
  it("shows the stat cells when everything is connected", async () => {
    const html = await renderSummary(
      summary({
        contextUsage: { contextWindow: 1000000, percent: 3.9, tokens: 39000 },
        usage: {
          extra_usage: {
            is_enabled: true,
            monthly_limit: 1000,
  ...
  it("colors heavy usage", async () => {
    const html = await renderSummary(
      summary({ usage: { five_hour: { resets_at: null, utilization: 95 } } }),
    );
