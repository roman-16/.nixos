...
- **Seams**: Land every change as the shape the code would have if it had always been that way. No second path beside the existing one, no flag or branch preserving the previous behavior, no bridging wrapper, no name that means "the new one" - one way to do the thing. Follow through: delete what the new design makes dead, rename what it makes wrong, carry every caller, test, doc and config into it. Anything left over from the previous design is a seam.
